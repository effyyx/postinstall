...
local function draw_rect_point(x0, y0, x1, y1, x2, y2, x3, y3, color, opt)
  ...
  s = s .. string.format("{\\p1}m %d %d l %d %d %d %d %d %d{\\p0}", x0, y0, x1, y1, x2, y2, x3, y3)
  draw_append(s)
end

local function draw_rect(x, y, w, h, color, opt)
  draw_rect_point(x, y, x + w, y, x + w, y + h, x, y + h, color, opt)
end
...
local function pbar_draw()
  ...
  if duration then
    ...
    if state.cached_ranges and opt.cachebar_height > 0 then
      ...
      draw_rect(0, dpy_h - (ch + ypos), dpy_w, ch, opt.cachebar_uncached_color)
  ...
  if state.pbar == PBAR_ACTIVE and duration then
    ...
    draw_rect(math.max(state.mouse.x - 1, 0), dpy_h - ypos, 2, ypos, opt.hover_bar_color)
    local fw = fs * 2 -- guesstimate ¯\_(ツ)_/¯
    local x = clamp(state.mouse.x, pad + fw, dpy_w - (pad + fw))
    draw_text(x, dpy_h - (ypos + fs), fs, "{\\an8}" .. hover_text, fopt)
    ...
    if cname then
      ...
      local x = clamp(state.mouse.x, pad + fw, dpy_w - (pad + fw))
      draw_text(x, dpy_h - (ypos + fs), fs, "{\\an8}" .. cname, fopt)
      ypos = ypos + fs + (fopt.bw * 2)
    end
    ...
    if not state.thumbfast.disabled then
      ...
      mp.commandv("script-message-to", "thumbfast", "thumb", hover_sec, x, y)
      ypos = ypos + th + pw

      -- L4: preview border
      if pw > 0 then
        local c = opt.preview_border_color
        draw_rect(x, y, tw, th, "1616167F", { bw = pw, bcolor = c })
        ypos = ypos + pw
      end
    end
  end
...
local function set_osc_margins()
  ...
  elseif utils.shared_script_property_set ~= nil then -- for older mpv versions
    utils.shared_script_property_set("osc-margins", string.format("%f,%f,%f,%f", 0, 0, 0, b))
  end
end
...
local function pbar_update(next_state)
  ...
  if
    dpy_w == 0
    or dpy_h == 0
    or state.pbar == next_state
    or state.pbar == PBAR_UNINIT
    or state.inhibit_count > 0
    or state.player_idle
  then
    return
  end
...
local function pbar_maximize(time)
  ...
  if state.pbar == PBAR_ACTIVE or time <= 0 then
    return
  end
...
local function set_chapter_list(kind, c)
  ...
  if c and #c > 0 then
    state.chapters = c
    table.sort(state.chapters, function(a, b)
...
local function init()
  ...
  state.userdata_avail = table_contains(mp.get_property_native("property-list"), "user-data")
